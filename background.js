...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "generateMindmap") {
    // Build a hierarchical mind map that covers the whole video.
    ...
    (async () => {
      try {
        ...
        if (tabs[0]) {
          ...
          // it can grab the playlist owner's name instead of the real channel,
          // and its description is truncated while the box is collapsed. We
          // fall back to the DOM only for fields the player didn't provide.
...
/**
 * Formats a whole number of seconds as MM:SS for prompts and display.
 */
...
/**
 * Derives the safe timing bounds for one video from its transcript and metadata.
 *
 * The transcript text is already prefixed with [M:SS] markers. Its LAST marker
 * is the most reliable signal of where the content actually ends — more
 * trustworthy than the duration metadata, which is sometimes missing or wrong.
 * We use the larger of (metadata duration, last transcript stamp). The late
 * threshold (75% in) forces a prompt to cover the WHOLE video instead of
 * front-loading everything near the start.
 */
function computeVideoBounds(transcriptText, videoDuration) {
  let lastTranscriptSeconds = 0;
  const stampMatches =
    String(transcriptText || "").match(/\[(\d+):(\d{2})\]/g) || [];
  if (stampMatches.length) {
    const last = stampMatches[stampMatches.length - 1].match(/\[(\d+):(\d{2})\]/);
    lastTranscriptSeconds = parseInt(last[1], 10) * 60 + parseInt(last[2], 10);
  }
  ...
  return {
    effectiveSeconds,
    durationFormatted: formatSeconds(effectiveSeconds),
    maxTimestampSeconds: effectiveSeconds,
    lateThresholdSeconds,
    lateThreshold: formatSeconds(lateThresholdSeconds),
  };
}
...
async function handleAnalyzeTranscript(
  ...
  try {
    ...
    // Derive the safe timing bounds once, in a shared helper, so the analysis
    // and the mind map treat the video length identically.
    const { durationFormatted, lateThreshold, maxTimestampSeconds } =
      computeVideoBounds(transcriptText, videoDuration);
...
/**
 * Bounds that keep a model-generated mind map renderable and cheap to draw.
 * They also cap output size, so one video can never produce a runaway tree.
 */
const MINDMAP_LIMITS = Object.freeze({
  maxDepth: 4,
  maxNodes: 120,
  maxLabelLength: 160,
});
...
/**
 * Rebuilds a mind map from untrusted model output.
 *
 * The response is treated as data only: labels are trimmed and capped,
 * out-of-range timestamps are dropped (the node itself stays), sibling
 * duplicates are removed, siblings are sorted by time, and the total depth and
 * node count are bounded so a runaway response can never reach the UI or
 * inflate render cost.
 *
 * @param {Object} raw - The parsed model response.
 * @param {Object} [options]
 * @param {number} [options.maxSeconds] - Largest valid timestamp in seconds.
 * @param {string} [options.fallbackTitle] - Title used when the model omits one.
 * @returns {{title: string, children: Array}|null} null when no usable tree.
 */
function validateMindmap(raw, { maxSeconds = 0, fallbackTitle = "" } = {}) {
  const safeMax =
    Number.isFinite(Number(maxSeconds)) && Number(maxSeconds) > 0
      ? Number(maxSeconds)
      : Number.MAX_SAFE_INTEGER;
  ...
  const safeLabel = (value) =>
    typeof value === "string"
      ? value
          .replace(/\s+/g, " ")
          .trim()
          .slice(0, MINDMAP_LIMITS.maxLabelLength)
      : "";
  const safeSeconds = (value) => {
    const seconds = Number(value);
    if (!Number.isFinite(seconds) || seconds < 0 || seconds > safeMax) {
  ...
  const buildLevel = (list, depth) => {
    if (!Array.isArray(list) || depth >= MINDMAP_LIMITS.maxDepth) return [];

    const seen = new Set();
    const level = [];
    for (const item of list) {
      if (!item || typeof item !== "object") continue;
      const label = safeLabel(item.label ?? item.title ?? item.text);
      if (!label) continue;
      ...
      level.push({
        label,
        seconds: safeSeconds(item.timestampSeconds),
    ...
    for (const item of level) {
      if (nodeCount >= MINDMAP_LIMITS.maxNodes) break;
      nodeCount += 1;
      nodes.push({
        label: item.label,
        timestampSeconds: item.seconds,
        timestamp: item.seconds === null ? "" : formatSeconds(item.seconds),
        children: buildLevel(item.children, depth + 1),
      });
    }
    return nodes;
  };

  const title = safeLabel(raw?.title) || safeLabel(fallbackTitle) || "Video";
  const children = buildLevel(raw?.children, 1);
  if (!children.length) return null;
  return { title, children };
}

/**
 * Sends the transcript to DeepSeek and builds a validated mind map.
 *
 * The prompt asks for one hierarchical tree covering the whole video. Every
 * returned node is treated as untrusted data and rebuilt by validateMindmap
 * before it can reach the UI.
 *
 * @returns {Object} - { success, mindmap } or { success: false, error }
 */
async function handleGenerateMindmap(
  ...
  try {
    ...
    debugLog("[YouTube Digest] Requesting mind map", settings.aiModel);
    const { text: responseText } = await requestAiCompletion({
      maxTokens: 8192,
      temperature: 0.2,
      responseFormat: { type: "json_object" },
    ...
    const mindmap = validateMindmap(parseLooseJson(responseText), {
      maxSeconds: maxTimestampSeconds,
    ...
    if (!mindmap) {
      return {
        success: false,
        error: "INVALID_MINDMAP",
        message: "The model returned no usable mind map. Please Retry.",
      };
    }
