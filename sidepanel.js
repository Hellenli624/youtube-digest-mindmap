...
// --- Mind map state ---
// Generated lazily when the Mind Map tab is first opened, then cached with the
// rest of the digest so revisits are instant and free.
let currentMindmap = null;
let isMindmapLoading = false;
let collapsedMindmapIds = new Set();
...
function setupEventListeners() {
  ...
  // Mind map toolbar
  document
    .getElementById("mindmapExpandBtn")
    ?.addEventListener("click", () => setAllMindmapCollapsed(false));
  document
    .getElementById("mindmapCollapseBtn")
    ?.addEventListener("click", () => setAllMindmapCollapsed(true));
  document
    .getElementById("mindmapFullViewBtn")
    ?.addEventListener("click", openMindmapFullView);
...
async function startDigest(videoId, videoUrl) {
  ...
  if (cached) {
    ...
    // Restore a previously built mind map, or clear the canvas for this video.
    if (cached.mindmap) {
      showMindmap(cached.mindmap);
      if (currentTranscriptMode !== "original") {
        void translateMindmapContent();
      }
    } else {
      currentMindmap = null;
      collapsedMindmapIds = new Set();
      setMindmapStatus("");
      renderMindmap();
    }
...
function switchTab(tabName) {
  ...
  if (tabName === "overview") {
    ...
    // Build lazily on first open, or re-render the cached tree.
    if (currentMindmap) {
      renderMindmap();
      if (currentTranscriptMode !== "original") {
        void translateMindmapContent();
      }
    } else if (!isMindmapLoading) {
      if (currentTranscriptTimestamped) {
        void triggerMindmap();
      } else {
        setMindmapStatus(
          "Waiting for the transcript before building the mind map.",
        );
      }
    }
  }
...
async function saveToCache(videoId) {
  ...
  try {
    ...
    const cacheData = {
      ...
      mindmap: currentMindmap, // May be null if not yet generated
...
/** Shows or clears the small status line above the mind map canvas. */
function setMindmapStatus(text) {
  const status = document.getElementById("mindmapStatus");
  if (!status) return;
...
/**
 * Collapses every branch below the first level so a long video opens as a
 * readable overview instead of a wall of nodes.
 */
function defaultCollapsedMindmapIds(tree) {
  if (typeof YTD_MINDMAP === "undefined" || !tree) return new Set();
  const layout = YTD_MINDMAP.buildLayout(tree, { compact: true });
  return new Set(
    layout.nodes
...
/** Adopts a freshly loaded tree and renders it in its default state. */
function showMindmap(tree) {
  currentMindmap = tree || null;
  collapsedMindmapIds = defaultCollapsedMindmapIds(currentMindmap);
  setMindmapStatus("");
  renderMindmap();
}

/**
 * Builds the mind map on demand. Runs once per video; later visits reuse the
 * cached tree, so no tokens are spent just by re-opening the tab.
 */
async function triggerMindmap() {
  if (isMindmapLoading || currentMindmap || !currentTranscriptTimestamped) {
    return;
  }
  isMindmapLoading = true;
  setMindmapStatus("Building mind map...");
  try {
    const result = await chrome.runtime.sendMessage({
    ...
    if (!result || !result.success) {
      setMindmapStatus(
        `Could not build the mind map: ${result?.error || "Unknown error"}. Open this tab again to Retry.`,
      );
      return;
    }

    currentMindmap = result.mindmap;
    collapsedMindmapIds = defaultCollapsedMindmapIds(currentMindmap);
    setMindmapStatus("");
    renderMindmap();
    await saveToCache(currentVideoId);
    ...
    console.error("[YouTube Digest Panel] Mind map error:", error);
    setMindmapStatus(`Could not build the mind map: ${error.message}`);
  } finally {
...
/**
 * Clones the tree with each label replaced by the text for the active language
 * mode. Original mode returns the tree untouched, so English costs nothing.
 * The structure (and therefore every node id) is preserved exactly.
 */
function localizeMindmapTree(tree, mode) {
  if (!tree || mode === "original" || typeof YTD_MINDMAP === "undefined") {
    return tree;
  }

  const labelForMode = (node, path) => {
    const label = YTD_MINDMAP.nodeLabel(node);
    const cacheKey = interfaceTranslationCacheKey(
      "mindmap",
      YTD_MINDMAP.segmentId(YTD_MINDMAP.getNodeId(path)),
      label,
    );
    const translated = interfaceTranslationCache.get(cacheKey);

    if (mode === "zh") {
      if (translated) return translated;
      return interfaceTranslationFailures.has(cacheKey)
...
/** Segments for the universal language control, keyed by stable node ids. */
function getMindmapTranslationSegments() {
  if (!currentMindmap || typeof YTD_MINDMAP === "undefined") return [];
  return YTD_MINDMAP.toSegments(currentMindmap);
}

/** Translates the mind map in small cached batches, revealing each as it lands. */
function translateMindmapContent() {
  const segments = getMindmapTranslationSegments();
  if (!segments.length) return Promise.resolve();
  return translateInterfaceSegments("mindmap", segments, () => {
...
/** Draws the current tree. Clicking a timestamped node seeks the video. */
function renderMindmap() {
  const canvas = document.getElementById("mindmapCanvas");
  if (!canvas) return;
  if (!currentMindmap || typeof YTD_MINDMAP === "undefined") {
    canvas.innerHTML = "";
    return;
  }

  const tree = localizeMindmapTree(currentMindmap, currentTranscriptMode);
  const layout = YTD_MINDMAP.buildLayout(tree, {
    compact: true,
    collapsedIds: collapsedMindmapIds,
  });
  YTD_MINDMAP.renderSvg(canvas, layout, {
    onNodeClick: (node) => {
      if (Number.isFinite(node.timestampSeconds)) {
        seekTo(node.timestampSeconds);
      }
    },
    onToggle: (nodeId) => toggleMindmapNode(nodeId),
  });
}

/** Expands or collapses one branch and redraws. */
function toggleMindmapNode(nodeId) {
  if (!nodeId) return;
  if (collapsedMindmapIds.has(nodeId)) {
    collapsedMindmapIds.delete(nodeId);
  } else {
    collapsedMindmapIds.add(nodeId);
  }
  renderMindmap();
}

/** Expands or collapses every branch at once. */
function setAllMindmapCollapsed(collapsed) {
  if (!currentMindmap || typeof YTD_MINDMAP === "undefined") return;
  if (collapsed) {
    const layout = YTD_MINDMAP.buildLayout(currentMindmap, { compact: true });
    collapsedMindmapIds = new Set(
...
/**
 * Opens the wide view in its own tab. It reads the map from the cached digest,
 * so no generation happens here.
 */
function openMindmapFullView() {
  if (!currentVideoId) return;
  chrome.tabs.create({
    url: `${chrome.runtime.getURL("mindmap.html")}?video=${encodeURIComponent(currentVideoId)}`,
  });
}
...
async function handleDisplayLanguageModeChange(mode) {
  ...
  // Redraw immediately so pending labels appear before their batches arrive.
  renderMindmap();
  ...
  } else if (activeTabName === "mindmap") {
    await translateMindmapContent();
